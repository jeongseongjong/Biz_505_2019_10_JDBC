...
public class ProductServiceV1 {
	...
	protected ProductDTO viewPDetail(String strPCode) {
		...
		/*
		 * 3항 연산자 조건식 ? true return : false return 조건식이 참이면 method가 true를 return하는것과 같고
		 * 그렇지 않으면 false를 return하는 것과 같다
		 * 
		 * 변수에 조건에 따라 다른값을 채우고 싶을때 변수 = 조건식 ? true : false s = a == 1 ? "참" : "거짓" 변수 a에
		 * 담긴 값이 1이면 변수 s에 "참"을 채우고 그렇지 않으면 "거짓"을 채워라
		 */
...
//		}
		...
		/*
		 * 만약 매입단가를 입력하지 않고 그냥 Enter만 눌렀다면 Integer.valueOf(strIPrice)에서 Exception 이 발생할
		 * 것이다. try로 {} 감싸진 코드에서 Exception이 발생을 하면 나머지 코드는 무시되고 catch() jump한다.
		 * 
		 * 이런기능을 역 이용하여 값을 입력하지 않거나, 문자열등을 포함하면 원래 proDTO의 iprice 값을 변경하지 않도록 한다.
		 */
		try {
		...
		/*
		 * proDTO의 필드변수는 각 항목을 입력받을때 Enter만 입력했다면 원래 DB table에서 가져온 값이 그대로 유지 될것이고
		 * 
		 * 값을 입력했다면 새로운 값으로 변경되어 있을것이다.
		 */

		/*
		 * Protected 현재 클래스를 상속받은 곳에서는 viewPDetail() method를 사용할 수 있지만 그렇지 않은곳에서는 접근 금지
		 * 
		 * extends로 상속받은 곳에서는 public 처럼 다른곳에서는 private 처럼 작동
		 */
